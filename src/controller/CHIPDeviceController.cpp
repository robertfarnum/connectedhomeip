                                       ...
                                       Optional<System::Clock::Timeout> timeout)
{
    ...
    JointFabricPki::Commands::AddICAC::Type request;
...
void DeviceCommissioner::OnICACertSuccessResponse(
    void * context, const chip::app::Clusters::JointFabricPki::Commands::AddICACResponse::DecodableType & data)
{
...
CHIP_ERROR DeviceCommissioner::SendICACSRRequestCommand(DeviceProxy * device, Optional<System::Clock::Timeout> timeout)
{
    MATTER_TRACE_SCOPE("SendICACSRRequestCommand", "DeviceCommissioner");
    ChipLogDetail(Controller, "Sending JointFabric request to %p device", device);
    VerifyOrReturnError(device != nullptr, CHIP_ERROR_INVALID_ARGUMENT);

    JointFabricPki::Commands::ICACSRRequest::Type request;

    ReturnErrorOnFailure(
        SendCommissioningCommand(device, request, OnICACSRResponse, OnJointFabricFailureResponse, kRootEndpointId, timeout));
...
void DeviceCommissioner::OnICACSRResponse(void * context, const JointFabricPki::Commands::ICACSRResponse::DecodableType & data)
{
    MATTER_TRACE_SCOPE("OnICACSRResponse", "DeviceCommissioner");
    ChipLogProgress(Controller, "Received JointFabric Information from the device");
    DeviceCommissioner * commissioner = reinterpret_cast<DeviceCommissioner *>(context);

    CommissioningDelegate::CommissioningReport report;
    report.Set<ICACSRResponse>(ICACSRResponse(data.NOCIssuerCSR));
    commissioner->CommissioningStageComplete(CHIP_NO_ERROR, report);
}
...

{
    ...
    case CommissioningStage::kSendICACSRRequest: {
        CHIP_ERROR err = SendICACSRRequestCommand(proxy, timeout);
        if (err != CHIP_NO_ERROR)
        {
            // We won't get any async callbacks here, so just complete our stage.
