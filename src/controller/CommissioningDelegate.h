...
enum CommissioningStage : uint8_t
{
    ...
    kSendICACSRRequest,          ///< Send ICACSRRequest (0x3E:12) command to the device
...
public:
    ...
    // NOC ICA CSR from the node. This is obtained from node in response to the ICACSRRequest command. In the
    // AutoCommissioner, this is automatically set from the report from the kSendICACSRRequest stage.
...
struct ICACSRResponse
{
    ICACSRResponse(ByteSpan newNocIssuerCsr) : nocIssuerCsr(newNocIssuerCsr) {}
...
public:
     ...
     * kSendNOC: None
     * kSendICACSRRequest: ICACSRResponse
    ...
    struct CommissioningReport
        : Variant<RequestedCertificate, AttestationResponse, CSRResponse, NocChain, ICACSRResponse, JointNOCIssuerCertificate,
